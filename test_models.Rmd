...
par(cex.axis=1.5)
for (i in 7:8){
  boxplot(bs.crps.mod2[[i]], bs.crps.mod3[[i]],bs.crps.mod4.3[[i]],bs.crps.mod10[[i]],bs.crps.mod5[[i]],bs.crps.mod6[[i]],bs.crps.mod7.3[[i]],bs.crps.mod11[[i]],main= paste0("Lead time ", sprintf("%02d",seq(6,48,by=6))[i],"h"), ylab="CRPS",las=2,names=c("Model 2","Model 3","Model 4.3" ,"Model 10","Model 5","Model 6","Model 7.3","Model 11"))
}
```
...
bs.mod2 <- bs.function(model2)
bs.mod3 <- bs.function(model3)
bs.mod5 <- bs.function(model5)
bs.mod6 <- bs.function(model6)

```

```{r}
bs.function.weights <- function(model){
 bs.test<-data.frame( thresholds = seq(2,24,by=2))
for(i in 1:8){
  for(j in 1:12){
  thresholds = seq(2,24,by=2)
...
bs.mod4.3 <- bs.function.weights(model4.3)
bs.mod7.3 <- bs.function.weights(model7.3)
```

##    Brier Plots 
```{r,fig.height=5,fig.width=6}
brier_testmodels<-function(threshold){
brier.mod <- data.frame(
  lead= seq(6,48,by=6),
  bs.mod2 = as.numeric(bs.mod2[(threshold/2),seq(3,17,by=2)]),
  bs.mod3 = as.numeric(bs.mod3[(threshold/2),seq(3,17,by=2)]),
  bs.mod5 = as.numeric(bs.mod5[(threshold/2),seq(3,17,by=2)]),
  bs.mod6 = as.numeric(bs.mod6[(threshold/2),seq(3,17,by=2)]),
  bs.mod4.3=as.numeric(bs.mod4.3[(threshold/2),seq(3,17,by=2)]),
  bs.mod7.3=as.numeric(bs.mod7.3[(threshold/2),seq(3,17,by=2)])
  )
rownames(brier.mod)<-paste0("lead", seq(6,48,by=6))
brier.mod <- melt(brier.mod,  id.vars = 'lead', variable.name = 'model')
g1 <- ggplot(brier.mod, aes(lead,value))+geom_line(aes(colour=model))+ggtitle(paste0("Threshold ",threshold))+labs(y="Brier Skill Score")+theme_grey(base_size=15)+scale_x_continuous(name="lead time",breaks=seq(6,48,by=6))
return(g1)}
...
```{r}
boot.brier2.test<-function(threshold,model){
...
boot.ss.mod2.test <- list(boot.brier2.test(14,model2),boot.brier2.test(16,model2),boot.brier2.test(18,model2))

boot.ss.mod3.test <- list(boot.brier2.test(14,model3),boot.brier2.test(16,model3),boot.brier2.test(18,model3))
boot.ss.mod5.test <- list(boot.brier2.test(14,model5),boot.brier2.test(16,model5),boot.brier2.test(18,model5))
boot.ss.mod6.test <- list(boot.brier2.test(14,model6),boot.brier2.test(16,model6),boot.brier2.test(18,model6))

save(boot.ss.mod2.test,boot.ss.mod3.test,boot.ss.mod5.test,boot.ss.mod6.test,boot.ss.mod4.3.test,boot.ss.mod7.3.test,file="boot.brier.test.RData")
```

```{r}
boot.brier.weight.test<-function(threshold,model){
bs.tot=vector(mode="list",length=8)
bs.test<-data.frame( thresholds = seq(2,24,by=2))
for(i in 1:8){
  bs.ss<-rep(NA,1000)
...
boot.ss.mod4.3.test <- list(boot.brier.weight.test(14,model4.3),boot.brier.weight.test(16,model4.3),boot.brier.weight.test(18,model4.3))

boot.ss.mod7.3.test <- list(boot.brier.weight.test(14,model7.3),boot.brier.weight.test(16,model7.3),boot.brier.weight.test(18,model7.3))
```


```{r}
load("boot.brier.test.RData")
par(mfrow=c(2,4))
for (i in 1:8){
  boxplot(boot.ss.mod2.test[[3]][[i]], boot.ss.mod3.test[[3]][[i]],boot.ss.mod5.test[[3]][[i]],boot.ss.mod6.test[[3]][[i]],boot.ss.mod4.3.test[[3]][[i]],boot.ss.mod7.3.test[[3]][[i]],main=paste0("Boot BSS, threshold 18, Lead",seq(6,48,by=6)[i]), xlab="Model",ylab="Brier",las=2,names=c("Model 2","Model 3","Model 5","Model 6","Model 4.3","Model 7.3"))
}
```
